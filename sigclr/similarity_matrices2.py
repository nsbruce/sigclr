...
from sigclr.dataset2 import SigCLRTorchSigNarrowband
...
def weights_to_similarity_matrices(weights_file: Path | None, save_latent_pkl: bool, save_similarities_npy: bool, save_html: bool, show: bool) -> None:
    ...
    if weights_file is not None:
        ...
        seed_everything(42)
        model = SigCLR(lr=0.001, temperature=0.0, weight_decay=1e-4)
    model.eval()
    ...
    for x, y in qa_dataset:
        ...
        x=ST.ComplexTo2D()(x)['data']['samples']
    ...
    if save_latent_pkl:
        with open('latent_vectors.pkl', 'wb') as f:
            pickle.dump((latent_vectors_h1, latent_vectors_h2, latent_vectors_z1, latent_vectors_z2), f)
